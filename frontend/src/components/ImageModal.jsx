    ...
    <div className="chat-overlay" onClick={onClose}>
      <div className="chat-window" style={{width: '900px', maxWidth:'95vw', height:'85vh', display: 'flex', flexDirection: 'column'}} onClick={e => e.stopPropagation()}>
        ...
        <div className="chat-header">
          <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
            ...
            <div style={{display:'flex', gap:'5px', alignItems:'center'}}>
              ...
              <span style={{fontSize:'11px', color:'#94a3b8', border:'1px solid #334155', padding:'1px 6px', borderRadius:'4px'}}>
                {caseData.subtype}
              </span>
            </div>
            {role === 'rural' && (
               <span style={{fontSize:'12px', background:'#334155', color:'#cbd5e1', padding:'2px 6px', borderRadius:'4px'}}>
                 Saved: {savings}
               </span>
            )}
          </div>
        ...
        <div className="chat-body" style={{display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px', overflowY: 'auto', background:'#020617', flex: 1}}>
          ...
          {/* Quality Metrics Bar */}
          {caseData.quality_metrics && (
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', background: '#1e293b', padding: '15px', borderRadius: '8px', border: '1px solid #334155', marginBottom: '10px'}}>
              <div style={{textAlign: 'center'}}>
                <div style={{fontSize: '11px', color: '#94a3b8', marginBottom: '4px'}}>PEAK SIGNAL-TO-NOISE (PSNR)</div>
                <div style={{fontSize: '18px', fontWeight: 'bold', color: '#22c55e'}}>{caseData.quality_metrics.psnr} dB</div>
              </div>
              <div style={{textAlign: 'center', borderLeft: '1px solid #334155', borderRight: '1px solid #334155'}}>
                <div style={{fontSize: '11px', color: '#94a3b8', marginBottom: '4px'}}>STRUCTURAL SIMILARITY (SSIM)</div>
                <div style={{fontSize: '18px', fontWeight: 'bold', color: '#3b82f6'}}>{caseData.quality_metrics.ssim}</div>
              </div>
              <div style={{textAlign: 'center'}}>
                <div style={{fontSize: '11px', color: '#94a3b8', marginBottom: '4px'}}>DIAGNOSTIC USABILITY SCORE</div>
                <div style={{fontSize: '18px', fontWeight: 'bold', color: '#a855f7'}}>{(caseData.quality_metrics.diagnostic_usability * 100).toFixed(1)}%</div>
              </div>
            </div>
          )}
